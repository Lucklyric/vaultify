...
use vault_cli::cli::Cli;
use vault_cli::interactive::InteractiveVault;
use vault_cli::utils::error_exit;
...
async fn run_interactive() {
    ...
    let vault_path = if args.len() == 3 && (args[1] == "-f" || args[1] == "--file") {
        ...
        if let Some(path) = vault_cli::utils::find_vault_file() {
            path
        } else {
            ...
            let create = if atty::is(atty::Stream::Stdin) && atty::is(atty::Stream::Stdout) {
                    ...
                    .interact()
                {
                    ...
                    Err(_) => {
                        eprintln!(
                            "{}",
                            "Unable to read input. Use 'vault init' to create a vault.".yellow()
                        );
                        false
                    }
                }
                ...
                eprintln!("{}", "Run 'vault init' to create a new vault, or run vault in an interactive terminal.".yellow());
                false
            };

            if create {
                // Create new vault file
                let content = vault_cli::parser::VaultParser::create_root_document();
                match std::fs::write(&vault_path, content) {
                    ...
                    Err(e) => {
                        error_exit(&format!("Failed to create vault: {}", e), 1);
                    }
                }
            } else {
                error_exit(
                    "No vault file available. Use 'vault init' to create one.",
                    1,
                );
            }
        }
